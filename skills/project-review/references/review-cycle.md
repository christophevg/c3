...
- **Simplicity Check (MANDATORY):** apply the design doctrine from the
  c3:code-reviewer persona (owner-proposal baseline; Wrapper/Pass-Through
  Test; Deletion, Abstraction and Library-First tests). Reject with
  `"simplicity: <reason>"` when it fails.
