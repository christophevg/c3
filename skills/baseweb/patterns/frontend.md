...
For browser-based testing, create test pages that are only loaded in development:

1. Add environment flag in `web.py`:
```python
...
2. Conditionally register in `pages/__init__.py`:
```python
from myapp.web import server, TEST_PAGE

if TEST_PAGE:
    server.register_component("tests.js", os.path.dirname(__file__), route="/tests")
```

3. Test page uses existing section (check `components/navigation.js` for available sections):
```javascript
const TestsPage = {
  navigation: {
    section: "info",  // Use existing section!
    icon: "bug_report",
    text: "Tests",
    path: "/tests",
    index: 99
...
<Page>
  <v-container>
    <v-card>
      <v-card-title><h2>🧪 Tests</h2></v-card-title>
      <v-card-text>
        <v-btn @click="runAllTests" color="primary">Run All Tests</v-btn>
  ...
  methods: {
    ...
    runAllTests: function() {
      this.results = [];
      // Run your tests and push results
      // this.results.push(this.pass("Test name", "Details"));
      // this.results.push(this.fail("Test name", "Error details"));
    }
  }
};
Navigation.add(TestsPage);
```

**Important**: 
- Never use a section that doesn't exist. Check `components/navigation.js` for available sections.
- Start server with `TEST_PAGE=true make run` to enable test page.
