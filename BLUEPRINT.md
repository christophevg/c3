...
- **Skills, three families:**
  ...
  - *Knowledge:* `python*`, `baseweb`, `vue`, `vuetify` (merged from v1–v4), `textual`, `rich`, `fire`, `pymongo`, `ollama`, `quart-webapp`, `readme`, `documentation`, `naming`, `api-design` (restored reference pack from slimmed api-architect), `prepare-for-exam`, `project-migrate`, `website-manage`
...
- 2026-09-01 — Triage Gate (owner decision after PM-dogfood review of log.txt): issue triage ends at an owner decision — the workflow pauses and presents the full triage table, the owner selects what starts/waits/drops. Bug classification alone never starts the bug flow; explicit per-round pre-approval skips the gate, silence never does. Batch triage is the default (one analyst pass, one comment-posting call, bug clusters proposed as one bundled fix PR). Applied: playbook Phase 0.3 + Bug Flow, issue-review-workflow, PM persona. This supersedes the 2026-08-31 "issues are urgent, do not ask" feedback — classification is the agent's proposal, the decision is the owner's.
- 2026-09-01 — Tool-gap discipline (same review): no capability probing — using a tool outside its documented scope to "see whether it works" is a workaround attempt; report after the FIRST failure. Missing sub-operations stop a phase after a consolidated report, with durable work (analysis, drafts) landed first. Instructions conflicts are resolved in one pass — decide, note, move on; only new evidence re-opens a settled call; balanced calls go to the owner. Applied: AGENTS.global.md (Tool Failure Protocol), PM persona (decision discipline + pre-flight capability check).
- 2026-09-01 — Memory-policy reinforcement (owner decision): C3 records learnings ONLY in active artifacts — agent definitions, skills, documentation (BLUEPRINT decision log is the project's running memory). No `memory/` files: nothing reads them today; a dedicated long-term-memory system remains a future item. Residual lessons-learned memory references healed (routing corrected to skills/agents/docs); release-manager persona gained the assign-AND-request-review PR rule (rescued from the one pre-existing feedback memory before removal); `memory/` directory contents deleted with owner approval.
- 2026-09-01 — Yoker github tool gap: `issue_edit` operation missing (labels/assignees/state on existing issues) — blocks all issue-label workflows; dev prompt delivered to owner for direct implementation. After it ships, C3's `yoker.toml` must add `"issue_edit"` to `[tools.github] allowed_operations` (default-deny, the #68 lesson).
...
- 2026-08-31 — Agent = persona + agent-interactions; skill = workflow. Orchestration doctrine (who the PM's team is, how to engage each agent, cross-cutting interaction rules like post-and-poll) is persona knowledge — its daily vocabulary — NOT workflow procedure. Batch A slimming conflated the two and stripped the PM's delegation map/lifecycle/post-and-poll/simplicity-gate into the playbook, where they didn't survive (verified by git archaeology: pilot persona 114fc9c had them; playbook never received them). Restored to persona; playbook keeps the full recipes as procedure record. Dogfood regression fixed: session-with-PM = managed mode, any project ask routes through Phase 0 (added unconditional line at playbook Phase 0); outside the playbook the PM still never executes directly — delegate or report the gap.
...
- 2026-08-31 (Batch D) — Batch D executed: type-frontmatter pass on all 27 remaining skills (22 knowledge, 5 workflow); all 48→47 skills now carry `type:`; 16 knowledge skills gained additive `## Related` sections (link hygiene); 27 skills promoted to the strict template ledger (9→36). Promotions are mechanical: backmatter + additive Related only, **no content stripping** (owner constraint for this batch). Mid-batch anomaly (two no-op appends + mangled tool emission; zero content damage) confirmed owner's stop-and-ask rule works — tool-args typo caused it, fixed by fresh-read discipline. Validator: 13/13 agents, 47 skills, 35 strict, 0 errors (commit `5f01286`).
- 2026-08-31 — `naming` skill retained (owner decision): fully migrated in Batch D, linked from develop-agent/develop-skill; re-evaluate like the other meta-skills only after real use.
- 2026-08-31 — `markdown-to-pdf` skill deleted (owner decision): superseded by the working `scripts/md_to_pdf.py` tool — owner explicitly chose script-over-skill ("we already have a working script"). Skill dir (3 files) removed; strict-ledger entry removed (36→35); README skill-table row removed; §4 roster entry updated. `scripts/md_to_pdf.py` + README + templates retained as the operative tool; its legacy `${CLAUDE_PLUGIN_ROOT}` refs queue for the Batch E docs pass. CHANGELOG/REPOSITORY-REVIEW history rows intentionally untouched.
- 2026-08-31 (Batch E, migration closed) — Batch E executed minimally (owner directive: small changes only, close the migration). Cut from scope: AGENTS.md, CONTRIBUTING.md (verified fine as-is). Done: README skill table regenerated to real roster (4 dead rows, 2 missing skills, counts corrected); TODO.md canonicalized (`## Unsorted` → `## Backlog` P1–P4, no Done; deferred migration items logged: memory system, vuetify ×4 merge review); CHANGELOG migration-close entry; scripts/README de-CLAUDE'd; `skills/research` gap closed (type: + Related + strict ledger → 36 skills strict). Owner-approved deletions: `backup-pre-batch-A/` (disk-only), `HANDOFF.md`, `MISSION.md`, `log.txt` (mission history preserved in git + BLUEPRINT). `.gitignore` anchoring fixed — root cause of the silent `skills/research` corpus exclusion (two tool lessons: unanchored `research` matches at any depth; `./research` matches nothing — bare `/research` anchors to root). Gitignore rules are part of the tool-corpus surface: validator walks the filesystem, search/list honor gitignore — keep them aligned. Migration is COMPLETE; further fixes flow through ongoing dogfooding (owner-driven) and the TODO backlog.
